...
| **A WebSocket for a job that does not exist, or one held open forever** | An unknown job id is closed with code 4404 instead of being parked on a ping loop holding a Redis connection; somebody else's job, under `AUTH_MODE=oauth`, is closed with 4403; a socket that has lived `EVENT_STREAM_TTL` is closed with 4408, because the stream it tails expired then; and a Redis blip on the socket's first read closes with 1013 (try again later) rather than telling the browser the job failed. The Redis pool is bounded (`REDIS_MAX_CONNECTIONS`). | A closed socket with a reason, rather than a gateway slowly running out of connections. |
